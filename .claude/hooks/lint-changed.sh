#!/usr/bin/env bash
# lint-changed.sh — PostToolUse(Write|Edit)
# Entry wrapper (verbatim invocation surface). Logic lives in lint-changed.py (Python 3 port of the
...
if [ -z "$PY" ]; then mkdir -p "$REPO/.claude/state" 2>/dev/null; echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) lint-changed SKIPPED: no working python3 on PATH (fail-visible)" >> "$REPO/.claude/state/bypass.log" 2>/dev/null; echo "lint-changed: no working python3 on PATH; hook skipped + logged to .claude/state/bypass.log" >&2; exit 0; fi
exec $PY "$DIR/lint-changed.py" "$@"
