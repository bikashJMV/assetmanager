...
export default function EmployeeForm({ prefill, onClose, onSubmit }: Props) {
  ...
  return (
    <div className="bg-surface-2 border border-base rounded-2xl w-full">
      <div className="flex items-center justify-between px-6 py-4 border-b border-base">
        <h2 className="font-semibold text-primary">{isEditing ? 'Edit Employee' : 'Add New Employee'}</h2>
        <button type="button" onClick={onClose} className="text-subtle hover:text-primary text-xl leading-none">x</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="px-6 py-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          />
          <div>
            <label className="block text-subtle text-xs mb-1">ERP / HR Status</label>
            <select
              value={form.is_active ? 'active' : 'inactive'}
              onChange={(e) => setForm((current) => ({ ...current, is_active: e.target.value === 'active' }))}
              className="w-full bg-surface border border-base rounded-lg px-3 py-2.5 text-primary text-sm outline-none focus:border-[color:var(--accent)] transition"
            ...
            <p className="text-[11px] text-subtle mt-1">This status is employee ERP/HR status, not asset inventory status.</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 border border-base text-muted py-2.5 rounded-lg hover:bg-surface-2 transition text-sm"
          >
            Cancel
          </button>
          <button
...
function Field({
  ...
  return (
    <div>
      <label htmlFor={fieldId} className="block text-subtle text-xs mb-1">
        {label}
      ...
      {type === 'select' ? (
        <select
          ...
          className="w-full bg-surface border border-base rounded-lg px-3 py-2.5 text-primary text-sm outline-none focus:border-[color:var(--accent)] transition disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {options.map((option) => (
