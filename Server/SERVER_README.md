...
This folder (`Server/`) is deployed as a **separate Vercel project** from the client.
...
All requests are routed to `app.py`, which re-exports `main.app`. Vercel's Python runtime handles Uvicorn internally.

### Vercel Environment Variables to Set

Set these in the **server Vercel project** dashboard:
...
- Set project root directory to `Server/` in the Vercel project settings.
- `FRONTEND_URL` **must** match the deployed client URL exactly (no trailing slash). Otherwise all server-generated QR codes will be broken.
- `ALLOWED_ORIGINS` **must** include the deployed client URL or browser requests will be blocked by CORS.
- For Vercel **preview deployments** (changing URLs per branch), fixed `ALLOWED_ORIGINS` won't work — consider switching to wildcard or regex-based CORS.

---

## Frontend Integration Note

The current **client runtime** (`Client/src/api.ts`) communicates **directly with Supabase** using the anon key via RLS-protected queries and RPCs. It does **not** call this FastAPI server at runtime.

The server is used for:
- **QR code generation** — creates base64 QR images pointing to the frontend scan URL
- **Data import/admin scripts** — `scripts/import_v2_from_sheet.py` uses the server's Supabase client
- **API-first workflows** — if you want to add a traditional REST API layer on top of Supabase

The `VITE_API_URL` variable in `Client/.env` is currently unused in the client codebase. It can be removed or kept as a placeholder if server integration is added later.

---

## Known Issues / Deployment Checklist

- [ ] `FRONTEND_URL` must be set in Vercel server project (not localhost) before deploying
- [ ] `ALLOWED_ORIGINS` must include the deployed client URL
- [ ] `SUPABASE_URL` and `SUPABASE_KEY` must be set — missing values cause a silent warning locally but a cryptic cold-start failure on Vercel
- [ ] `BACKEND_API_KEY` should be set for any internet-facing deployment
- [ ] `ENV=production` should be set in Vercel to suppress error detail leaking
- [ ] Client `Client/.gitignore` should include `.env` (currently missing) to prevent credential commits
- [ ] `settings.py` should ideally `raise ValueError` instead of `print("WARNING")` for missing Supabase credentials to fail fast at startup
- [ ] `import_v2_from_sheet.py` is a standalone script — do **not** invoke it from inside Vercel serverless functions
