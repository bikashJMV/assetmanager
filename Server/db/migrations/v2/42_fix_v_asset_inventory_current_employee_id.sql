-- 42_fix_v_asset_inventory_current_employee_id.sql
--
-- Final fix for employee assigned-assets visibility bug.
--
...
-- employees_self_read RLS policy (`auth.uid() = auth_user_id`) applies to that JOIN.
-- PostgreSQL evaluates the JOIN condition AND the RLS condition together, so for
-- non-admin callers the joined employee row is filtered out and e.id is NULL —
-- even for assets assigned to the calling employee.
--
...
-- The e.* columns (name, email, is_active, department) remain from the LEFT JOIN
-- and will be NULL for non-admin callers viewing other employees' data (intended).
--
-- This migration supersedes migrations 40 and 41 (partial attempts).
...
select
  ...
  ca.assignment_id,
  ca.assigned_at,
  -- FIX: Use ca.employee_id (from asset_assignments, using(true)) instead of e.id
