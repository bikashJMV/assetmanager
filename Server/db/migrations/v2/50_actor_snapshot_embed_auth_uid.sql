-- 50_actor_snapshot_embed_auth_uid.sql
-- Nested SECURITY DEFINER calls can yield NULL auth.uid() inside fn_asset_event_actor_snapshot(),
-- so assign/return/lifecycle showed "System / public". Embed auth.uid() in the payload from the
-- outer RPC when no employees row exists, via coalesce(full_employee_snapshot, minimal_by_jwt).
-- Client: parseActorSnapshot coerces UUID fields to string (Supabase may return non-strings).

set search_path = public;

-- Full snapshot from employees.id, else minimal { actor_id: auth.uid(), ... nulls } when JWT present.
create or replace function fn_actor_snapshot_for_event_payload(p_session_employee_pk uuid)
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function fn_actor_snapshot_for_event_payload(uuid) from public;
...
begin
  ...
  v_actor_snapshot := coalesce(
    fn_actor_snapshot_from_employee_pk(v_actor_employee_id),
    fn_actor_snapshot_for_event_payload(null)
  );
...
begin
  ...
  v_actor_snap := fn_actor_snapshot_for_event_payload(v_session_emp);
